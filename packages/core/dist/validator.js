export function validateDocument(doc) {
    ...
    if (!d.viewport || typeof d.viewport !== "object") {
        ...
        const vp = d.viewport;
        if (typeof vp.width !== "number")
            errors.push({ path: "viewport.width", message: "must be number" });
        if (typeof vp.height !== "number")
            errors.push({ path: "viewport.height", message: "must be number" });
    }
    ...
    if (!d.root) {
        ...
        errors.push(...validateComponent(d.root, "root"));
    }
    if (d.components && typeof d.components === "object") {
        for (const [name, def] of Object.entries(d.components)) {
            errors.push(...validateComponent(def, `components.${name}`));
        }
    }
    return errors;
}
function validateComponent(comp, basePath) {
    const errors = [];
    ...
    if (comp.on && typeof comp.on === "object") {
        for (const [event, handler] of Object.entries(comp.on)) {
            errors.push(...validateEventHandler(handler, `${basePath}.on.${event}`));
        }
    }
    return errors;
}
function validateEventHandler(handler, basePath) {
    const errors = [];
