...
export const Dashboard: FC<{ comp: Component; resolvedProps: Record<string, unknown> }> = ({ comp, resolvedProps }) => {
  ...
  useEffect(() => {
    ...
    comp.children?.forEach((child) => {
      const p = child.props ?? {};
      if (child.ref) init[child.ref] = { colSpan: (p.colSpan as number) ?? 1, rowSpan: (p.rowSpan as number) ?? 1 };
    });
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent, childRef: string) => {
    ...
    const startState: ResizeState = { ref: childRef, startColSpan: cur.colSpan, startRowSpan: cur.rowSpan, startX: e.clientX, startY: e.clientY };
    resizeRef.current = startState;

    const move = (ev: MouseEvent) => {
      const r = resizeRef.current;
      if (!r) return;
      const dCol = Math.round((ev.clientX - r.startX) / 160);
      const dRow = Math.round((ev.clientY - r.startY) / 120);
      setSpans((p) => ({
        ...p,
        [r.ref]: {
          colSpan: Math.max(1, Math.min(columns, r.startColSpan + dCol)),
          rowSpan: Math.max(1, Math.min(6, r.startRowSpan + dRow)),
        },
      }));
    };
    ...
    const up = () => {
      ...
      if (r) {
        const f = spans[r.ref];
        if (f && (f.colSpan !== r.startColSpan || f.rowSpan !== r.startRowSpan)) {
          emit("resize", { colSpan: f.colSpan, rowSpan: f.rowSpan });
        }
      }
        ...
        return (
          <div key={ref} style={{ gridColumn: `span ${span.colSpan}`, gridRow: `span ${span.rowSpan}`, position: "relative", minHeight: 100 }}>
            ...
            <div
              ...
              style={{ position: "absolute", right: 4, bottom: 4, width: 20, height: 20, cursor: "nwse-resize", zIndex: 10, opacity: 0, transition: "opacity 0.2s" }}
...
export const Widget: FC<{ comp: Component }> = ({ comp }) => {
  ...
  // dataIntent is a non-standard field on Component used by the dashboard runtime
  const dataIntent = (props as unknown as { dataIntent?: { refreshInterval?: number } }).dataIntent;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column" }}>
      {items.map((item, i) => {
        ...
        const dc = dotColors[item.color ?? "accent"] ?? "var(--air-accent)";
        return (
